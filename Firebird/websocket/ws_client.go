...
func (cli *Client) runDefaultClient() {
	...
	*/
	//message := []byte("{\"sub\":\"market.btcusdt.kline.1min\", \"id\":\"id10\"}")
	...
	*/
	...
	message := []byte("{\"sub\":\"market.eosusdt.depth.step1\"}")
	err = c.WriteMessage(websocket.TextMessage, message)
	if err != nil {
		log.Error("write err :", err)
	}
	//message = []byte("{\"sub\":\"market.btcusdt.depth.step1\"}")
	//err = c.WriteMessage(websocket.TextMessage, message)
	//if err != nil {
	//	log.Println("write err :", err)
	//}
	...
	*/
	...
	for {
		...
		if len(bids) > 0 {
			symbols := strings.Split(depthData.Ch, ".")
			fmt.Println(symbols[1], bids[0])
			service.NotifySymbolPrice(symbols[1], bids[0][0])
		}
...
func createClient(addr string) {
	clientNameNum++
	client := NewClient(addr, cast.ToString(clientNameNum))
	go client.RunClient()
}
